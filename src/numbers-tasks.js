...
function getAverage(value1, value2) {
  const bigIntValue1 = BigInt(value1);
  const bigIntValue2 = BigInt(value2);

  const average = (bigIntValue1 + bigIntValue2) / BigInt(2);
...
function toExponential(number, fractionDigits) {
  ...
  const absForm = Math.abs(number);
  const expForm = absForm.toExponential(fractionDigits);
...
function toFixed(number, fractionDigits) {
  const absForm = Math.abs(number);
  const floatForm = absForm.toFixed(fractionDigits);
